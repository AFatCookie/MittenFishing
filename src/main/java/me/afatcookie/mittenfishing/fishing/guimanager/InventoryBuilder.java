...
public class InventoryBuilder {
    ...
    /**
     * meant to reload configuration
     *
     * @param reloadConfig whether to reload config
     */
    public InventoryBuilder(boolean reloadConfig) {
        if (reloadConfig) {
            FileConfiguration config = MittenFishing.getInstance().getConfig();
            config.addDefault("maingui.name", "Fishing Menu");
            config.addDefault("maingui.rows", 5);
            config.addDefault("maingui.filler", "WHITE");
            config.addDefault("maingui.fillstyle", "fillAll");
            config.addDefault("maingui.items", "remove this and fill in below");
            config.options().copyDefaults(true);
            MittenFishing.getInstance().saveConfig();
        }
    }
